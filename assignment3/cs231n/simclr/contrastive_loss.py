...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        l1_denom = torch.tensor([sim(z_k, z) for z in out[np.arange(2*N)!=k]])
        l2_denom = torch.tensor([sim(z_k_N, z) for z in out[np.arange(2*N)!=k+N]])

        l1 = -((sim(z_k, z_k_N) / tau).exp() / (l1_denom / tau).exp().sum()).log()
        l2 = -((sim(z_k_N, z_k) / tau).exp() / (l2_denom / tau).exp().sum()).log()
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    similarity = sim_matrix[range(2*N), [*range(N, 2*N), *range(0, N)]]
    ...
    numerator = (similarity / tau).exp().to(device)
    ...
    loss = - ((numerator / denom).log().sum() / (2*N))
