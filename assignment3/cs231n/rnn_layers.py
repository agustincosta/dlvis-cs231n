...
def rnn_step_backward(dnext_h, cache):
    ...
    aux = np.dot(prev_h, Wh) + np.dot(x, Wx) + b
    #dtanh = (4*np.cosh(aux)**2)/((np.cosh(2*aux) + 1)**2) * dnext_h #dim (N, H)
    dtanh = dnext_h * (1 - np.square(next_h))

    dx = np.dot(dtanh, Wx.T) #dim (N, D) = (N, H) * (H, D)
    dprev_h = np.dot(dtanh, Wh.T) #dim (N, H) = (N, H) * (H, H)
    dWx = np.dot(x.T, dtanh) #dim (D, H) = (D, N) * (N, H)
    dWh = np.dot(prev_h.T, dtanh) #dim (H, H) = (H, N) * (N, H)
    db = np.sum(dtanh.T, axis=1) #dim (H,)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      #Slice input for corresponding time
      x_step = np.squeeze(x[:, t, :])
      #Step forward
      h_next, cache_t = rnn_step_forward(x_step, h_prev, Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
      dh_t = np.squeeze(dh[:, t, :]) + dh0
      dx_t, dh0, dWx_t, dWh_t, db_t = rnn_step_backward(dh_t, cache[t])
