...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # A few hints:                                                             #
        #  1) You first have to embed (self.embedding()) your caption and add      #   
        #     positional encoding (self.positional_encoding()).                    #
        #     You then have to project the image features into the same dimensions #
        ...
        #Caption encoding
        tokenized = self.embedding(captions)
        pos_encod = self.positional_encoding(tokenized)

        #Image feature encoding and adding new dimention
        proj_features = self.visual_projection(features).unsqueeze(1)

        #Create mask
        tgt_mask = torch.ones(T, T)
        tgt_mask = torch.tril(tgt_mask)
        tgt_mask = tgt_mask.flip(-1)

        scores = self.transformer(pos_encod, proj_features, tgt_mask)
